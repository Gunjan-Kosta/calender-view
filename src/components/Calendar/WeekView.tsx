...
import { getWeekDays, formatTime, isToday } from "../../utils/date.utils";
...
const WeekViewComponent: React.FC<WeekViewProps> = ({
  ...
  return (
    <div
      className="bg-white border border-neutral-200 shadow-sm rounded-lg flex overflow-auto"
      role="region"
      aria-label={`Week view starting ${currentDate.toLocaleDateString()}`}
    >
      <div className="w-20 flex-shrink-0">
        ...
        {timeSlots.map((hour) => (
          <div
            key={hour}
            className="h-24 text-right pr-2 pt-1 text-xs text-neutral-500 border-r border-neutral-200"
          >
            {hour === 0
              ? "12 AM"
              : hour < 12
        ...
        {weekDays.map((day) => {
          ...
          return (
            ...
            >
              ...
              <div className="relative h-[2880px]">
                {" "}
                {/* 24 * 4 * 30px */}
                {timeSlots.map((hour) => (
                  <div
                    key={hour}
                    className="h-24 border-b border-neutral-100"
                  ></div>
                ))}
                {dayEvents.map((event) => {
