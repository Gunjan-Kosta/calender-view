...
const AgendaViewComponent: React.FC<AgendaViewProps> = ({
  currentDate,
  events,
  onEventClick,
}) => {
  ...
  const virtualItemsMeta = useMemo(() => {
    ...
    return agendaEventsByDay.map((group) => {
      const height =
        DAY_GROUP_HEADER_HEIGHT +
        group.events.length * EVENT_ITEM_HEIGHT +
        GROUP_SPACING;
  ...
  const totalHeight =
    virtualItemsMeta.length > 0
      ? virtualItemsMeta[virtualItemsMeta.length - 1].offset +
        virtualItemsMeta[virtualItemsMeta.length - 1].height
      : 0;
  ...
  const getVisibleGroups = () => {
    ...
    for (const item of virtualItemsMeta) {
      if (
        item.offset + item.height > scrollTop &&
    ...
    return (
      <div className="bg-white border border-neutral-200 shadow-sm rounded-lg p-10 text-center animate-fade-in">
        ...
        <p className="text-neutral-500 mt-2">
          There are no events scheduled on or after{" "}
  ...
  const renderGroup = (
    group: { date: Date; events: CalendarEvent[] },
    ...
    return (
      <div
        style={style}
        className="flex flex-col md:flex-row md:gap-6 p-4 md:p-6"
      >
        <div className="md:w-40 flex-shrink-0 mb-2 md:mb-0">
          <h3
            className={clsx(
              "font-bold text-lg",
              isTodaysDate ? "text-primary-600" : "text-neutral-900"
            )}
          >
          ...
          >
            {dayEvents.map((event) => (
              <li
                ...
                tabIndex={0}
                aria-label={`Event: ${event.title}, from ${formatTime(
                  new Date(event.startDate)
                )} to ${formatTime(new Date(event.endDate))}`}
              >
                <div
                ...
                <div className="flex-grow min-w-0">
                  <p className="font-semibold text-neutral-800 group-hover:text-primary-600 truncate">
                    {event.title}
                  </p>
                  <p className="text-sm text-neutral-500">
  ...
  return (
    <div
      ...
      role="region"
      aria-label={`Agenda view starting ${currentDate.toLocaleDateString()}`}
      style={{
        maxHeight: "70vh",
        overflowY: "auto",
      }}
